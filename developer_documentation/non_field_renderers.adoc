...
|`:link`, `:button`, `:action_button`, `:info_button`, `:success_button`, `:warning_button`, `:error_button` or `:back_button`.
|`:link`

|options - :text_size
|Only applies when `style` is `:button`, `:back_button`, `:info_button`, `:success_button`, `:warning_button`, `:error_button` or `:action_button`. Uses a scale of 1 to 6 with 1 being standard and 6 being the largest.
|1 to 6
|1
...
|options - :nested
|True if the tabs are nested inside another TabSet
|Boolean
|False
...
Call `make_rows_for_up_to_6_cols(no_of_columns)` to add enough rows for the given number of columns.
...
page.layout_grid do |grid|
  ...
  # This will create the best combination of rows and columns:
  grid.make_rows_for_up_to_6_cols(array.length)
  array.each { |txt, val| grid.add_kpi_card text, val }
end
----
...
The `:chart_type` will instantiate the correct subclass of `Chart` (e.g. `:donut` => `DonutChart`, `:bar` => `BarChart`, `:stacked_bar` => `StackedBarChart`)
...
|options - :colour_field
|Colour field (StackedBar chart)
|String
|None. Required for applicable chart types

|options - :colour_title
|Colour field title
|String
|A humanised version of `:colour_field`

|===

==== Expectations per chart type

Although each chart type is generated in the same way (`add_chart(:type, options)`), the different sub-types have distinct expectations for the `options` hash.

Most of these options reference field names in records of the `data` array option.

NOTE: the `:data` option provides an array of data for the chart to use. If there is no data for a chart, you can pass in an empty array, or nil.

==== :bar

x_field, y_field:: the fields representing the two axes.
x_type, y_type:: Optional, defaults to 'nominal' for X and 'quantitative' for Y.
x_title:: Label for the x-axis, defaults to the value of the `x_field`.

==== :stacked_bar

As for `:bar` above, with these additional options:

colour_field:: The field that is used to assign the quantitative value to a particular bar.
colour_title:: The Legend to display for the stacks, defaults to the value of the `colour_field`.

==== :donut

q_field:: The name of the quantitative field.
n_field:: The name of the nominal field.
n_title:: Label for the nominal field, defaults to the value of `n_field`.

=== HeatmapTable

Render an SVG table of data with "heatmap" colours applied to cells based on their value in relation to the full set of values.

The query for this data should return rows with three fields - one for the row name, one for the column name and one for the value to show in the cell defined by the intersection of the row and column. In the example below those fields are: `:puc_code`, `:size_count` and `:standard_cartons`.

[source,ruby]
----
grid.specify_row(1)
table = Crossbeams::Layout::HeatmapTable.prepare_table_from_data(data) do |opts|
  opts.row_title = 'PUC'
  opts.row_key = :puc_code
  opts.col_key = :size_count
  opts.val_key = :standard_cartons
end
grid.add_heatmap_table table,
                       title: 'Size distribution per week (std cartons)',
                       human_numbers: true,
                       row_title: 'PUC',
                       col_title: 'Size',
                       cell_title: 'STD cartons'
----

[NOTE]
====
The helper method on the class (`prepare_table_from_data(data, exclude_zero_total_rows: true, order_by: :total_desc)` takes query data and prepares the crosstab table required for a heatmap table.

The method receives the data, and two options (`exclude_zero_total_rows` which defaults to true and will exclude all rows where the row total is zero and `:order_by` which defaults to `:total_desc`, and can be `:row` or `:total_asc` - to order the rows by the row value, or by the row total, ascending or descending).

It is called with a block where you specify options for the preparation of the table.

[source,ruby]
----
table = Crossbeams::Layout::HeatmapTable.prepare_table_from_data(data) do |opts|
  # REQUIRED options:
  opts.row_title = 'PUC'             # The column header at top left
  opts.row_key   = :puc_code         # A valid key for a data row
  opts.col_key   = :size_count       # A valid key for a data row
  opts.val_key   = :standard_cartons # A valid key for a data row

  # OPTIONAL options (here showing the defaults)
  opts.row_agg_title = 'TOTAL'       # The column header at top right
  opts.foot_title    = 'TOTAL'       # The column footer at bottom left
  opts.agg_func      = :sum          # :sum, :avg or :avg_perc
  opts.col_fmt       = nil           # nil or a lambda function.
  opts.row_sort_func = nil           # nil or a lambda function.
  opts.sort_by       = ->(c) { c == 'None' ? 0 : c.to_i }
end
----

`agg_func`:: defines how to caluclate row aggregation for the end of the row and for the footer. `:sum` will sum all values, while `:avg` will calculate the row average, and `:avg_perc` will calculate an average for the row and express that as a percentage of the grand total.

`col_fmt`:: is used to format the column headings. When nil, the column value will be displayed as-is. If a lambda function is provided, this will be called with the column value. e.g. `->(c) { "W#{c.delete_prefix('0')}" }` will turn `['01', '02', '10']` into `['W1', 'W2', 'W10']`.

`row_sort_func`:: is used to override the default row order (when `order_by: :row` is specified). The default is to sort as strings, but if the row is defined by an integer value, you can sort using `->(c) { c.to_s.match?(/^\D+$/) ? c : c.to_s.rjust(6, '0') }` to get `50, 60, 100, 120` instead of `100, 120, 50, 60`.

`sort_by`:: is a lambda function that is used to sort the column values. The default is to convert the values to integers and sort in ascending order.
====
...
|options - :human_numbers
|If true, the cell values will be humanized: 1000 => 1k, 1000 000 => 1m
|Boolean
|false

|options - :zero_as
|An optional string to display in cells instead of "0", or when the value is nil
|String
|nil

|options - :row_title
|The name of the 1st Y column (row names)
|String
|The cell value of the first column of the first row of data

|options - :col_title
|The group name for the columns
|String
|nil
