...
[NOTE]
====
The helper method on the class (`prepare_table_from_data(data, exclude_zero_total_rows: true)` takes query data and prepares the crosstab table required for a heatmap table.

The method receives the data, and an option (`exclude_zero_total_rows` which defaults to true and will exclude all rows where the row total is zero).
...
table = Crossbeams::Layout::HeatmapTable.prepare_table_from_data(data) do |opts|
  ...
  opts.agg_func      = :sum          # :sum or :avg_perc
  opts.col_fmt       = nil           # nil or a lambda function.
  opts.sort_by       = ->(c) { c == 'None' ? 0 : c.to_i }
end
----

`agg_func`:: defines how to caluclate row aggregation for the end of the row and for the footer. `:sum` will sum all values, while `:avg_perc` will calculate an average for the row and express that as a percentage of the grand total.
