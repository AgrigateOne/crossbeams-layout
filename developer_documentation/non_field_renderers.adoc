...
[NOTE]
====
The helper method on the class (`prepare_table_from_data(data, exclude_zero_total_rows: true, order_by: :row)` takes query data and prepares the crosstab table required for a heatmap table.

The method receives the data, and two options (`exclude_zero_total_rows` which defaults to true and will exclude all rows where the row total is zero and `:order_by` which defaults to `:row`, and can be `:total_desc` or `:total_asc` - to order the rows by the row value, or by the row total, ascending or descending).
