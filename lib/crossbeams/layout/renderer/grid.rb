    ...
    module Renderer
      ...
      class Grid < Base # rubocop:disable Metrics/ClassLength
        ...
        def self.header(grid_id, caption, options = {}) # rubocop:disable Metrics/PerceivedComplexity
          print_section = <<~HTML
            <label style="margin-left: 10px;">
              <button onclick="crossbeamsGridEvents.onBtPrint('#{grid_id}')" class="#{SC.css_class(:button_tertiary)}" title="Print"><svg class="cbl-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M4 16H0V6h20v10h-4v4H4v-4zm2-4v6h8v-6H6zM4 0h12v5H4V0zM2 8v2h2V8H2zm4 0v2h2V8H6z"/></svg>
              </button>
            </label>
          HTML
          ...
          bookmark_button = if options[:bookmark_row_on_action]
                              <<~HTML
                                <label style="margin-left: 10px;">
                                <button type="button" class="crossbeams-row-bookmark #{SC.css_class(:button_tertiary)}" title="Jump to bookmarked row" hidden>
                                  <svg class="cbl-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2 2c0-1.1.9-2 2-2h12a2 2 0 0 1 2 2v18l-8-4-8 4V2z"/></svg>
                                </button>
                                </label>
                              HTML
