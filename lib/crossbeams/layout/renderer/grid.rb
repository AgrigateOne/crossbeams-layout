    ...
    module Renderer
      ...
      class Grid < Base # rubocop:disable Metrics/ClassLength
        ...
        def self.header(grid_id, caption, options = {}) # rubocop:disable Metrics/PerceivedComplexity
          ...
          print_section = <<~HTML
            <button onclick="crossbeamsGridEvents.onBtPrint('#{grid_id}')" class="#{SC.css_class(:button_tertiary)} w-full flex gap-2" title="Print">
              <svg class="#{SC.css_class(:icon_s)}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          ...
          bookmark_button = if options[:bookmark_row_on_action]
                              <<~HTML
                                <button type="button" class="crossbeams-row-bookmark #{SC.css_class(:button_tertiary)} w-full flex gap-2" hidden>
                                  <svg class="#{SC.css_class(:icon_s)}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2 2c0-1.1.9-2 2-2h12a2 2 0 0 1 2 2v18l-8-4-8 4V2z"/></svg> Jump to last-visited row
                                </button>
                              HTML
                            else
                              ''
          ...
          actions_button = <<~HTML
            <div class="crossbeams-dropdown-button w-fit relative inline-block">
              ...
              <div class="crossbeams-dropdown-content flex hidden absolute right-0 top-12 z-10 bg-white">
          ...
          grid_state_button = <<~HTML
            <div class="crossbeams-dropdown-button w-fit relative inline-block">
              <button type="button" class="#{SC.css_class(:button_tertiary)}">
                  <!--svg class="#{SC.css_class(:icon_s)} inline" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" style="-ms-transform: rotate(360deg); -webkit-transform: rotate(360deg); transform: rotate(360deg);" preserveAspectRatio="xMidYMid meet" viewBox="0 0 20 20"><path d="M10 3a7 7 0 1 0 .001 13.999A7 7 0 0 0 10 3z" fill="#626262"/></svg--> Column state
              <svg class="#{SC.css_class(:icon_s)} inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
              </svg>
              </button>

              <div class="crossbeams-dropdown-content hidden absolute right-0 top-12 z-10 bg-white">
                <button type="button" class="#{SC.css_class(:button_tertiary)} w-full flex gap-2" onclick="crossbeamsGridEvents.gridStateSave('#{grid_id}')" title="Save grid column state">
                    #{Icon.new(:save).render} Save column state
                </button>

                <button type="button" class="#{SC.css_class(:button_tertiary)} gridStateLoad w-full flex gap-2 !hidden" onclick="crossbeamsGridEvents.gridStateLoad('#{grid_id}')" title="Load previously-saved grid column state">
                ...
                <button type="button" class="#{SC.css_class(:button_tertiary)} gridStateClear w-full flex gap-2 !hidden" onclick="crossbeamsGridEvents.gridStateClear('#{grid_id}')" title="Back to default grid column state">
                ...
                <button type="button" class="#{SC.css_class(:button_tertiary)} gridStateDelete w-full flex gap-2 !hidden" onclick="crossbeamsGridEvents.gridStateDelete('#{grid_id}')" title="Discard saved grid column state">
                  <svg class="#{SC.css_class(:icon_s)}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" focusable="false" width="1em" height="1em" style="-ms-transform: rotate(360deg); -webkit-transform: rotate(360deg); transform: rotate(360deg);" preserveAspectRatio="xMidYMid meet" viewBox="0 0 100 100"><path d="M15.194 59.995l69.732-.074v-.014a2.493 2.493 0 0 0 2.361-2.489a2.487 2.487 0 0 0-.802-1.823L51.834 21.02l-.004.004a2.484 2.484 0 0 0-1.902-.892a2.494 2.494 0 0 0-2.02 1.041l-34.46 34.535a2.498 2.498 0 0 0 1.746 4.287z" fill="#626262"/><path d="M87.308 77.253l-.01-9.803v-.05h-.005a2.534 2.534 0 0 0-2.534-2.485v-.006l-69.751.074v.042a2.53 2.53 0 0 0-2.293 2.516c0 .033.008.063.01.096l.01 9.477c-.006.074-.022.145-.022.22a2.528 2.528 0 0 0 2.311 2.511v.023l69.751-.074a2.536 2.536 0 0 0 2.534-2.539l-.001-.002z" fill="#626262"/></svg> Remove saved state
              </button>
              </div>
            </div>
          HTML
